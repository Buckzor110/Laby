...
def Format(name, value):
    print(name.ljust(36, ' '), str(value).rjust(8, ' '))
...
def main():
    ...
    while Ok == '':
        ...
        Format('Всегда твёрдых согласных букв', count1)
        Format('Всегда мягких согласных букв', count2)
        Format('Прочих символов в тексте', count3)
        ...
        for word in mass:
            ...
            if word.find('ко') != (-1) or word.find('ер') != (-1):
                count1 += 1
                print('- ' + word + ' сч1=' + str(count1))
            if word.startswith('циф'):
                count2 += 1
        ...
        Format('Слов, содержащих \"ко\"/\"ер\"', count1)
        Format('Слов, начинающихся с \"циф\"', count2)
        Format('Слов, заканчивающихся на \"ть\"', count3)

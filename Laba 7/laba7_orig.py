...
def H(matrix):
    ...
    for row in range(matrix['rows']):
        for col in range(matrix['cols']):
            summa = summa + matrix['mass'][row][col]
...
def V(matrix):
    ...
    for row in range(rows):
        for col in range(cols // 2):
            value = matrix['mass'][row][col]
            matrix['mass'][row][col] = matrix['mass'][row][cols - col - 1]
            matrix['mass'][row][cols - col - 1] = value
...
def Z(matrix):
    ...
    # Проверка, что в матрице достаточно элементов
    if rows < 2 or cols < 2:
        print('Недостаточно элементов для зигзага')
        return
